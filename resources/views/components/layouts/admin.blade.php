                            ...
                            <x-slot:trigger>
                                <div class="flex items-center space-x-2 p-2 rounded-lg hover:bg-base-200 cursor-pointer">
                                    <div class="w-8 h-8 rounded-full bg-primary text-primary-content flex items-center justify-center font-semibold text-sm">
                                        {{ request()->attributes->get('user')->initials() }}
                                    </div>
                            ...
                            <x-mary-menu class="menu menu-md w-56 z-50">
                                <div class="px-3 py-3 border-b border-base-200">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <div>
                                            <div class="font-semibold text-sm">{{ request()->attributes->get('user')->name }}</div>
                                            <div class="text-xs opacity-70">{{ request()->attributes->get('user')->email }}</div>
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="w-64 min-h-full bg-base-100 border-r border-base-200 flex flex-col">
                    ...
                    <div class="flex-1 p-4">
                        <x-mary-menu class="menu menu-vertical w-full">
                            <x-mary-menu-item link="{{ route('admin.dashboard') }}" :active="request()->routeIs('admin.dashboard')" wire:navigate>
                                <x-mary-icon name="o-home" class="w-5 h-5" />
                                Dashboard
                            </x-mary-menu-item>
                            <x-mary-menu-item link="{{ route('admin.companies') }}" :active="request()->routeIs('admin.companies*')" wire:navigate>
                                <x-mary-icon name="o-building-office" class="w-5 h-5" />
                                Companies
                            </x-mary-menu-item>
                            <x-mary-menu-item link="{{ route('admin.teams') }}" :active="request()->routeIs('admin.teams*')" wire:navigate>
                                <x-mary-icon name="o-user-group" class="w-5 h-5" />
                                Teams
                            </x-mary-menu-item>
                            <x-mary-menu-item link="{{ route('admin.users') }}" :active="request()->routeIs('admin.users*')" wire:navigate>
